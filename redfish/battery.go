...
// Battery shall represent a battery for a Redfish implementation.
type Battery struct {
	common.Entity
	...
	Assembly string
	// CapacityActualAmpHours shall contain the actual maximum capacity of this battery in amp-hours.
	CapacityActualAmpHours float64
	// CapacityActualWattHours shall contain the actual maximum capacity of this battery in watt-hours.
	CapacityActualWattHours float64
	// CapacityRatedAmpHours shall contain the rated maximum capacity of this battery in amp-hours.
	CapacityRatedAmpHours float64
	// CapacityRatedWattHours shall contain the rated maximum capacity of this battery in watt-hours.
	...
	// resource.
	Links string
	// Location shall contain location information of this battery.
	Location string
	...
	Manufacturer string
	// MaxChargeRateAmps shall contain the maximum charge rate of this battery in amps.
	...
	// MaxDischargeRateAmps shall contain the maximum discharge rate of this battery in amps.
...
func (battery *Battery) Update() error {
	...
	original.UnmarshalJSON(battery.rawData)
...
func ListReferencedBatterys(c common.Client, link string) ([]*Battery, error) {
	...
	for _, batteryLink := range links.ItemLinks {
		battery, err := GetBattery(c, batteryLink)
		if err != nil {
			collectionError.Failures[batteryLink] = err
		} else {
