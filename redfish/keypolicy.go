...
// NVMeoFSecurityProtocolType is This enumeration shall list the NVMe security protocols that a key is allowed to
// use.
...
func ListReferencedKeyPolicys(c common.Client, link string) ([]*KeyPolicy, error) {
	...
	for _, keypolicyLink := range links.ItemLinks {
		keypolicy, err := GetKeyPolicy(c, keypolicyLink)
		if err != nil {
			collectionError.Failures[keypolicyLink] = err
		} else {
			result = append(result, keypolicy)
		}
	}
