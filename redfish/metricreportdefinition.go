...
// CalculationAlgorithmEnum is This type shall specify the function to apply to the list of metric properties.
...
// CollectionTimeScope is This type shall specify the time scope of the corresponding metric values.
...
const (
	...
	// value shall specify the duration between the startup of resource and timestamp.
	StartupIntervalCollectionTimeScope CollectionTimeScope = "StartupInterval"
)

// MetricReportDefinitionType is This type shall specify when the metric report is generated.
...
// ReportActionsEnum is This type shall specify the actions to perform when a metric report is generated.
...
// ReportUpdatesEnum is This type shall indicate how the service handles subsequent metric reports when a metric
// report exists.
...
// UnmarshalJSON unmarshals a Metric object from the raw JSON.
func (metric *Metric) UnmarshalJSON(b []byte) error {
	type temp Metric
	var t struct {
		temp
	}
...
type MetricReportDefinition struct {
	...
	// ReportTimespan shall contain maximum timespan that a metric report can cover.
	...
	// specified by the RecurrenceInterval property within Schedule. If MaxOccurrences property within Schedule is
...
func ListReferencedMetricReportDefinitions(c common.Client, link string) ([]*MetricReportDefinition, error) {
	...
	for _, metricreportdefinitionLink := range links.ItemLinks {
		metricreportdefinition, err := GetMetricReportDefinition(c, metricreportdefinitionLink)
		if err != nil {
			collectionError.Failures[metricreportdefinitionLink] = err
		} else {
			result = append(result, metricreportdefinition)
		}
	}
...
// Update commits updates to this object's properties to the running system.
func (wildcard *Wildcard) Update() error {

	// Get a representation of the object's original state so we can find what
	// to update.
	original := new(Wildcard)
	original.UnmarshalJSON(wildcard.rawData)
	...
	originalElement := reflect.ValueOf(original).Elem()
	currentElement := reflect.ValueOf(wildcard).Elem()

	return wildcard.Entity.Update(originalElement, currentElement, readWriteFields)
}
